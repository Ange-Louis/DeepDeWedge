...
from .fit_n2v_model import LitUnet3D
...
def refine_tomogram(
    tomo_files: Annotated[
        List[Path],
        typer.Option(
            help="List of paths to tomograms (mrc files) reconstructed from the tilt series or movie frames."
        ),
    ],
    ...
    with torch.no_grad():
        for t_file in tomo_files:
            ...
            if output_dir is not None:
                basename, _ = os.path.splitext(os.path.basename(t_file))
