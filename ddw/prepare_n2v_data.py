...
def prepare_data(
    ...
    standardize_full_tomos: Annotated[
        bool,
        typer.Option(
            help="If 'True', the tomo and tomo1 tomograms will be standardized (mean=0, std=1) before extracting the subtomograms. This is useful for tomograms with low voxel itensities, and DDW can fail when processing such tomograms wihtout standardization."
        ),
    ] = False,
    ...
    if verbose:
        print(f"Starting subtomogram extraction from {len(tomo_files)} tomogram(s).")
    fitting_counter, val_counter = 0, 0
    for k, (tomo_file, mask_file) in enumerate(
        zip(tomo_files, mask_files)
    ):
        tomo = load_mrc_data(tomo_file).float()
        if standardize_full_tomos:
            print(
                f"Standardizing tomogram '{tomo_file}' before extracting sub-tomograms."
            )
            tomo -= tomo.mean()
            tomo /= tomo.std()
        else:
            ...
            if std < 1e-3:
                print(f"\
                    WARNING: Standard deviation of '{tomo_file}' is low ({std}), which may lead to issues during model fitting!\
        ...
        if mask_file is not None and verbose:
            print(
                f"Masking selected {len(selected_subtomo_ids)}/{len(subtomos)} subtomos extracted from tomogram {k}"
            )
        subtomos = [
            subtomo for k, subtomo in enumerate(subtomos) if k in selected_subtomo_ids
        ]
        start_coords = [
        ...
        #     subtomo_size=subtomo_size,
        #     target_sample_size=math.floor(len(subtomos) * val_fraction),
        #     max_tries=3,
        #     verbose=False,
        # )
        # if len(val_ids) < math.floor(len(subtomos) * val_fraction):
        #     print(
        #         f"WARNING: Could not sample {round(val_fraction*100, 2)}% of all subtomos for validation due to overlap with the fitting data. "
        #         f"Continuing with {round((len(val_ids)/len(subtomos))*100, 2)}% (i.e. {len(val_ids)}) validation subtomos."
        #     )
        num_val_subtomos = math.ceil(len(subtomos) * val_fraction)
        val_ids = (
            random.Random(seed).sample(range(len(subtomos)), num_val_subtomos)
            if num_val_subtomos > 0
            else []
        )
        fitting_ids = [k for k in range(len(subtomos)) if k not in val_ids]

        for idx in sorted(fitting_ids):
            torch.save(
                subtomos[idx].clone(), f"{fitting_subtomo_dir}/subtomo/{fitting_counter}.pt"
            )
            fitting_counter += 1

        for idx in sorted(val_ids):
            torch.save(
                subtomos[idx].clone(), f"{val_subtomo_dir}/subtomo/{val_counter}.pt"
            )
            val_counter += 1
...
def setup_subtomo_dir(subtomo_dir, project_dir, overwrite, verbose):
    ...
    os.makedirs(f"{fitting_subtomo_dir}/subtomo/", exist_ok=False)
    os.makedirs(f"{val_subtomo_dir}/subtomo/", exist_ok=False)
